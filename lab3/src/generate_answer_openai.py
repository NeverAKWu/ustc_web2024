...
react_chain = ReActChain(llm=llm)
print("load llm ok!")
...
template = """任务描述：
你是一名专业的法律知识问答助手。请使用以下检索到的上下文片段来回答问题。回答过程中请一步步思考。步骤：
理解问题：首先，明确用户提出的问题是什么。
检索信息：在提供的上下文片段中寻找相关信息。
分析信息：逐步分析检索到的信息，判断哪些内容与问题相关。你需要列出具体的事实、法律条文或案例而不是给出文档编号。例如：“根据公安部-机动车登记规定2021-12-17：第八十六条 规定……”
形成答案：根据分析，构建一个准确的答案。
验证答案：确保答案直接来源于上下文片段。
回答问题：如果在上下文中找不到答案，请直接回答“未找到相关答案”。
Question: {question}
Context: {context}
Answer: """
...
for question in questions:
    ...
    answer.append(texts)
    answer_llm.append(llm(question).content)
    answer_llm_RAG.append(rag_chain.invoke(question))


filename = "./question_answer_4o_CoT.csv"
