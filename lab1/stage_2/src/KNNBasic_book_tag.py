...
#相似度方法：MSD:0.7964~0.8002；pearson：0.7964
#1.对某些重复打分，时间新的权重高(但是没有多少重复打分)---ndcg变化不明显，甚至略有下降
#2. 过滤掉评分过少的用户和评分过少的书:划分数据集后再次出现的评分过少怎么解决？剔除该部分数据
...
# 按 Book 分组，并聚合标签
df_grouped = df_cpoy_book.groupby('Book').agg({
...
similarity_matrix1 = cosine_similarity(X) #计算出书本-书本相似度矩阵

# 获取所有书本的唯一 ID
unique_books = df_cpoy_book['Book'].unique()
...
missing_books = set(df_cpoy_book['Book']) - set(book_to_index.keys())
if missing_books:
...
print(filtered_df)
#过滤掉打分少于两个的
books_counts = filtered_df.groupby('Book')['Rate'].count()
valid_books = books_counts[books_counts >= 2].index
filtered_df1 = filtered_df[filtered_df['Book'].isin(valid_books)]
print("book filtered")
print(filtered_df1)
...
# 选择需要的列
df_aggregated = df_aggregated[['User', 'Book', 'Rate','Tag']]
...
print(sorted_predictions)
sorted_predictions.to_csv('book_pred_score.csv', index=False)
